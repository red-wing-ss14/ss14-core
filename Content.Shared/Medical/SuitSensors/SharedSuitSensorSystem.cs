...
public abstract class SharedSuitSensorSystem : EntitySystem
{
    ...
    [Dependency] protected readonly SharedIdCardSystem _idCardSystem = default!;
    ...
    public SuitSensorStatus? GetSensorState(Entity<SuitSensorComponent?, TransformComponent?> ent)
    {
        ...
        var status = new SuitSensorStatus(GetNetEntity(sensor.User.Value), GetNetEntity(ent.Owner), userName, userJob, userJobIcon, userJobDepartments, sensor.Mode);
        switch (sensor.Mode)
        {
            case SuitSensorMode.SensorBinary:

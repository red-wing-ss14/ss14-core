...
public sealed partial class GuideMicrowaveEmbed : PanelContainer, IDocumentTag, ISearchableControl, IGuidebookEntryAnchor, IPrototypeRepresentationControl
{
    ...
    public IPrototype? AnchorPrototype => RepresentedPrototype;
    ...
    private void GenerateSolidIngredients(FoodRecipePrototype recipe)
    {
        foreach (var (product, amount) in recipe.IngredientsSolids.OrderByDescending(p => p.Value))
        {
            ...
            IngredientsGrid.AddChild(new GuideEntityEmbed(product, false, false, registerAsGuideAnchor: false)); // RW

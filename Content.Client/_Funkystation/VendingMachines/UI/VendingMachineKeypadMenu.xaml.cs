...
public sealed partial class VendingMachineKeypadMenu : FancyWindow
{
    ...
    // flavor text
    private readonly string[] _successPhrases = [ "[ УДАЧИ ]", "[ СПС! ]", "[ НЯМ! ]", "[ ВЫДАН ]" ];
    ...
    private readonly Color _screenTextColor = Color.FromHex("#33ff33");
    private readonly Color _screenDimColor = Color.FromHex("#1a551a");
    private readonly Color _screenErrorColor = Color.FromHex("#ff6b6b");
    ...
    public void Populate(List<VendingMachineInventoryEntry> inventory, bool enabled)
    {
        ...
        for (var i = 0; i < inventory.Count; i++)
        {
            ...
            slot.SetItem(entry.ID, code, entry.Amount, entry.Amount == 0, entry.DisplayPrice, CanAfford(entry.DisplayPrice));
    ...
    public void UpdateAmounts(List<VendingMachineInventoryEntry> inventory, bool enabled)
    {
        ...
        for (var i = 0; i < inventory.Count; i++)
        {
            ...
            _slots[i].SetPrice(inventory[i].DisplayPrice, CanAfford(inventory[i].DisplayPrice));
        }
    ...
    public void SetBalance(int? balance)
    {
        ...
        if (balance is { } value)
        {
            BalanceLabel.Text = Loc.GetString("vending-machine-keypad-price-value", ("amount", value));
            BalanceLabel.FontColorOverride = _screenTextColor;
        }
        else
        {
            BalanceLabel.Text = NoValueText;
            BalanceLabel.FontColorOverride = _screenDimColor;
        }

        foreach (var slot in _slots)
        {
            slot.SetPrice(slot.Price, CanAfford(slot.Price));
        }
    ...
    {
        return price <= 0 || _balance is not { } balance || price <= balance;
    }
    ...
    private void SubmitBuffer()
    {
        ...
        if (index >= 0 && index < _slots.Count)
        {
            ...
            // check funds
            if (!CanAfford(slot.Price))
            {
                ShowFeedback(false, Loc.GetString("vending-machine-keypad-no-funds"));
                return;
            }
    ...
    private void ShowFeedback(bool success, string? failureText = null)
    {
        _bufferLetter = null;
        _bufferNumber = null;
        ...
        else
        {
            ...
            BufferLabel.Text = failureText ?? "[ ПУСТО ]";
    ...
    private void UpdatePriceReadout()
    {
        if (TryGetBufferedSlot(out var slot))
        {
            PriceLabel.Text = slot.Price <= 0
                ? Loc.GetString("vending-machine-keypad-price-free")
                : Loc.GetString("vending-machine-keypad-price-value", ("amount", slot.Price));
            PriceLabel.FontColorOverride = CanAfford(slot.Price) ? _screenTextColor : _screenErrorColor;
            return;
        }

        PriceLabel.Text = NoValueText;
        PriceLabel.FontColorOverride = _screenDimColor;
    }

    private bool TryGetBufferedSlot([NotNullWhen(true)] out VendingMachineGridSlot? slot)
    {
        slot = null;

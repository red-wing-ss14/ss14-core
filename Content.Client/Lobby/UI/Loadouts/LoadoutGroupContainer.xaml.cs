...
using Content.Shared._Amour.Loadouts.Effects; // Amour
...
public sealed partial class LoadoutGroupContainer : BoxContainer
{
    ...
    public void RefreshLoadouts(HumanoidCharacterProfile profile, RoleLoadout loadout, ICommonSession session, IDependencyCollection collection)
    {
        ...
        var validProtos = _groupProto.Loadouts
            ...
            .Where(p =>
            {
                // For personal fluff and booster loadouts, validate that user has access
                var hasUserNameEffect = p.Effects.Any(e => e is UserNameLoadoutEffect);
                var hasBoostyEffect = p.Effects.Any(e => e is BoostyTierLoadoutEffect);

                if (!hasUserNameEffect && !hasBoostyEffect)
                    return true;

                return loadout.IsValid(profile, session, p.ID, collection, out _);
            });
         // Amour edit end
    ...
    private LoadoutContainer CreateLoadoutUI(LoadoutPrototype proto, HumanoidCharacterProfile profile, RoleLoadout loadout, ICommonSession session, IDependencyCollection collection, LoadoutSystem loadoutSystem)
    {
        ...
        if (pressed)
        {
            cont.SourceType = DetermineLoadoutSourceType(proto.ID, loadout, profile, protoMan);
        }
        // Amour edit end
        ...
        IPrototypeManager protoMan)
    {
        ...
        // Check loadout effects to determine source type
        if (protoMan.TryIndex(loadoutId, out var loadoutProto))
        {
            foreach (var effect in loadoutProto.Effects)
            {
                if (effect is UserNameLoadoutEffect)
                    return LoadoutSourceType.PersonalFluff;

                if (effect is BoostyTierLoadoutEffect)
                    return LoadoutSourceType.BoostySubscription;
            }
        }

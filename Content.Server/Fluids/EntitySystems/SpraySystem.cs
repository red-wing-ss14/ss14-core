...
public sealed class SpraySystem : SharedSpraySystem
{
    ...
    public override void Spray(Entity<SprayComponent> entity, MapCoordinates mapcoord, EntityUid? user = null)
    {
        ...
        if (TryComp<UseDelayComponent>(entity, out var useDelay) // Goob edit
        ...
        if ((clickMapPos.Position - sprayerMapPos.Position).Length() < 0.5f)
        {
            ...
            if (newSolution.Volume > 0)
            {
                ...
                RaiseLocalEvent(user.Value, new SprayUserImpulseEvent(-impulseDirection.Normalized() * entity.Comp.PushbackAmount));
